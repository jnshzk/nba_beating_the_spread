...
## Productionalization
As a final step in this project, I wanted to productionalize the winning model.  To do so, I set up an automated script to run daily during the NBA season via Cron jobs.  The automated script will refresh the existing training data with each team's latest statistics, retrain the model, and evaluate the available bets for that day's games.  If any bets meet the confidence threshold criteria (i.e. the betting spread and predicted spread have a discrepancy of at least 20 points), an email alert will be sent out to all users on the mailing list.  This email alert will inform users of the teams playing, the betting spread, the predicted spread, and a recommendation on which team to wager on.
